...
class Endpointman_Templates
{
	...
	function edit_template_display_files($id, $custom, $namefile = "")
	{
    	...
    	$dReturn = array();
		if ($custom == 0) {
			$sql = "SELECT endpointman_model_list.max_lines, endpointman_model_list.model as model_name, endpointman_template_list.global_custom_cfg_data,  endpointman_product_list.config_files, endpointman_custom_configs.original_name, endpointman_product_list.short_name, endpointman_product_list.id as product_id, endpointman_model_list.template_data, endpointman_model_list.id as model_id, endpointman_template_list.* FROM endpointman_product_list, endpointman_model_list, endpointman_template_list, endpointman_custom_configs WHERE endpointman_product_list.id = endpointman_template_list.product_id AND endpointman_template_list.model_id = endpointman_model_list.id AND endpointman_template_list.id = " . $id;
		} else {
...
$row = sql($sql, 'getRow', DB_FETCHMODE_ASSOC);
	...
	if (!is_array($row)) {
		...
		$config_files_og = explode(",", $row['config_files']);
			...
			
		{
			...
			$alt_configs_list = array_unique($alt_configs_list, SORT_REGULAR);
